...
系统必须让任何已认证用户在公开 bucket 上开 issue（标题 + markdown 正文），并让 issue 作者和 bucket owner 评论并关闭它。Issues 在公开 buckets 上必须可被匿名读取。私有 buckets 不得接受非 owner 的 issues。
...
系统必须让已认证用户以 pull request 的形式对公开 bucket 提出变更，其中包含标题、描述和一份提议的内容 diff。Bucket owner 必须能够审阅、merge 或拒绝。Merge 必须把该变更作为归属于 PR 作者的一次 git commit 应用到 bucket，并且必须在应用前重新跑资产格式校验和配额检查。
...
系统必须让已认证用户把任何公开 bucket 中的资产安装（复制）进自己拥有的 bucket。安装后的副本必须记录 provenance（源 bucket、源 commit、安装时间），并且必须通过目标 bucket 的配额检查。
